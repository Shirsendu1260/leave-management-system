...
<head>
    ...
    <title>LMS • Total Viewed Applications</title>
    ...
    <div class="d-flex flex-column justify-content-center align-items-center">
        <nav class="w-100 navbar navbar-expand-lg bg-body-tertiary shadow-sm shadow-bottom">
            <div class="container-fluid px-3">
                <div class="navbar-brand mx-2 fw-bold"><span class="custom-bg-text">L</span>MS / TOTAL VIEWED
                    APPLICATIONS</div>
            ...
            <div class="card shadow">
                <div class="custom-bg rounded-top">
                    <div class="d-flex justify-content-between m-0 p-3 text-white">
                        <h5 class="m-0 d-flex align-items-center">Total Viewed Applications</h5>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped" id="myTable">
                            ...
                            <tbody>
                                ...
                                @foreach($totalapplications as $totalapplication)
                                <tr valign="middle">
                                    <td>{{ $totalapplication->id }}</td>
                                    <td>{{ $totalapplication->uid }}</td>
                                    <td>{{ $totalapplication->name }}</td>
                                    <td>
                                        @if($totalapplication->photo != '' && file_exists(public_path() .
                                        '/uploads/users/'
                                        .
                                        $totalapplication->photo))
                                        <img src="{{ url('/uploads/users/' . $totalapplication->photo) }}" width="50"
                                    ...
                                    <td>{{ $totalapplication->email }}</td>
                                    <td>{{ $totalapplication->gender }}</td>
                                    <td>{{ $totalapplication->dept }}</td>
                                    <td>{{ $totalapplication->from }}</td>
                                    <td>{{ $totalapplication->to }}</td>
                                    <td>{{ $totalapplication->day }}</td>
                                    <td>{{ $totalapplication->reason }}</td>
                                    <td class="@if($totalapplication->approved == 'Yes') text-success @endif
                                        @if($totalapplication->approved == 'No') text-danger @endif">{{
                                        $totalapplication->approved }}</td>
                                </tr>
                                @endforeach
                                @endif
                            </tbody>
